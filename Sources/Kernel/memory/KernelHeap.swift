...
extension KernelHeap {
    func load(
        ...
        logger.log(string: "KernelHeap: loaded at \(UInt(bitPattern: startAddress)) with \(size) bytes")
    }
}
...
extension KernelHeap {
    ...
    func allocate(
        ...
        switch alignment {
        case 4:  logger.log(staticString: "KernelHeap: allocate: alignment == 4")
        case 8:  logger.log(staticString: "KernelHeap: allocate: alignment == 8")
        case 16: logger.log(staticString: "KernelHeap: allocate: alignment == 16")
        case 32: logger.log(staticString: "KernelHeap: allocate: alignment == 32")
        default: logger.log(staticString: "KernelHeap: allocate: unknown alignment")
        }
        //logger.log(staticString: "KernelHeap: allocate: allocated X bytes with Y alignment")
...
extension KernelHeap {
    func deallocate(_ pointer: UnsafeMutableRawPointer?) {
        // TODO: implement
        logger.log(staticString: "KernelHeap: deallocate: NOT IMPLEMENTED")
    }
}
...
extension KernelHeap {
    func verify(amount: Int) {
        logger.log(staticString: "KernelHeap: verify: starting...")
        ...
        logger.log(string: "KernelHeap: verify: testObject allocated at memory address: \(UInt(bitPattern: testObjectPointer))")
        ...
        logger.log(string: "KernelHeap: verify: Array at \(UInt(bitPattern: testArray.baseAddress!)) contains \(testArray.count) elements.")
        
        // test alignment (crucial for SIMD)
        guard let rawPtr = unsafe malloc(16) else {
            logger.log(staticString: "KernelHeap: verify: failed to malloc(16)")
            return
        }
        let addr = UInt(bitPattern: rawPtr)
        if addr % 16 == 0 {
            logger.log(string: "KernelHeap: verify: 16-byte alignment confirmed at \(addr)")
        } else {
            logger.log(string: "KernelHeap: verify: ALIGNMENT FAILURE at \(addr)")
        }
    }
}
