...
func initIDT() {
    logger.log(staticString: "IDT: initializing...")

    registerIDTInterrupts()

    logger.log(staticString: "IDT: loading idtr...")
    ...
    unsafe idt.mutableSpan.withUnsafeBytes { ms in
        guard let bs = ms.baseAddress else {
            logger.log(staticString: "IDT: idt.mutableSpan.withUnsafeBytes baseAddress == nil")
            return
        }
        ...
        logger.log(staticString: "IDT: calling load_idtr...")
        unsafe load_idtr(&idtr)
    }

    logger.log(staticString: "IDT: initialized")
}